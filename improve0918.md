# HTMLpoint 개선 실행 계획 — 2026-09-18

작성일: 2026-09-18 · 상태: 실행 준비 · 기본 실행 모델: Luna Max · 제품 구현 미착수

비교 분석 원문: [2026-09-18 비교·개선 계획](docs/2026-09-18-comparison-and-improvement-plan.md). 개선 실행은 이 문서의 모델 제약과 단계별 완료 기준을 함께 따른다.


## 실행 하네스와 모델 사용 제약

이 절은 아래 개선 계획을 실행할 때 지켜야 할 운영 규칙이다. 구현·검증·통합의 기본 담당은 **Luna Max**다. 여기서 Luna Max는 Luna 모델에 추론 강도 Max를 적용한 설정을 뜻한다. Astra와 Sol은 필요한 리뷰 또는 막힘 해결에만 제한적으로 사용한다.

이번 문서 작성은 실행 규칙을 정리하는 작업이다. 문서를 작성했다는 사실이 현재 세션의 모델을 변경하거나 개선 구현을 시작했다는 뜻은 아니다.

### 역할과 허용 범위

| 역할 | 허용 모델 | 허용 작업 | 제약 |
| --- | --- | --- | --- |
| 주 실행자·코디네이터 | **Luna Max** | 작업 분해, 구현, 테스트, 결과 통합, 진행 기록, 완료 판정 | 일반 구현을 Astra/Sol에 넘기지 않는다 |
| 모든 Subagent | **Luna Max만 허용** | 독립적인 구현·테스트·조사 등 명시적으로 배정된 하위 작업 | 중첩 Subagent에도 같은 제한 적용. 다른 모델 또는 낮은 추론 강도로 자동 대체 금지 |
| 제한적 리뷰·막힘 자문 | **Astra 또는 Sol 중 하나** | 지정한 변경의 리뷰 또는 구체적 장애 원인 분석 | 한 의뢰에 한 작업·한 모델. 제품 코드 수정·통합·일반 구현 대행 금지 |

**Astra/Sol을 Subagent로 생성하지 않는다.** 상위 모델 자문은 실행 환경이 제공하는 별도 최상위 세션 또는 명시적인 단일 모델 호출로 수행한다. 도구 이름이 달라도 부모 실행자가 생성·관리하는 하위 에이전트라면 Subagent 제한을 적용한다. 별도 이름의 하위 에이전트로 만들어 이 제한을 우회하지 않는다.

자문용 별도 호출을 지원하지 않는 환경에서는 Astra/Sol을 Subagent로 대신 호출하지 않는다. 의뢰 자료를 준비하고 해당 항목만 외부 자문 대기로 기록하며, Luna Max로 가능한 독립 작업을 계속한다. 실제 호출 수단이 필요해질 때 그 제약을 구체적으로 보고한다.

### 한 작업에 한 상위 모델

1. 구현 작업에는 안정적인 작업 ID를 부여한다. 리뷰·막힘 자문은 원래 구현 작업 ID에 연결한다.
2. 자문이 필요하면 **Astra 또는 Sol 하나만** 선택하고 작업 기록에 고정한다. Astra와 Sol의 동시 호출, 순차 교차 검토, 같은 문제의 중복 의뢰는 금지한다.
3. 같은 구현 작업에 대한 리뷰와 막힘 분석도 이미 지정한 상위 모델을 사용한다. 작업 ID를 쪼개거나 이름을 바꿔 다른 모델에 다시 의뢰하지 않는다.
4. 한 의뢰에는 하나의 검토 목적·범위·완료 조건만 담는다. 전체 프로젝트나 여러 구현 작업을 한꺼번에 맡기지 않는다.
5. 상위 모델 의뢰는 한 번에 한 건만 진행한다. 독립적인 Luna Max 작업은 파일 충돌이 없을 때 계속할 수 있다.
6. 의뢰 범위 안의 추가 증거·답변 보완은 같은 의뢰 ID와 같은 모델로 처리한다. 완료되면 자문 세션을 종료하고 실행권을 Luna Max가 유지한다.
7. 지정 모델이 실패하거나 사용할 수 없더라도 다른 상위 모델로 자동 전환하지 않는다. 모델 배정을 바꿔야 한다면 기존 기록을 남기고 사용자에게 명시적으로 변경을 요청한다.

이 제한은 **구현 담당 Luna Max와 자문 담당 상위 모델의 역할 구분**이다. 상위 모델이 승인해야만 모든 작업을 완료할 수 있다는 뜻은 아니다.

### 자문을 호출하는 조건

- **리뷰:** 원본 DOM/CSS/script 보존, 저장·복구, 개체 ID와 연결선 참조, 격리된 출력 처리, PPTX 변환의 누락·중복처럼 영향이 큰 변경에서 독립 검토가 필요한 경우. 먼저 Luna Max가 구현·관련 테스트·검토 자료를 준비한다.
- **막힘:** 실패를 재현하고 로그·최소 입력·시도한 해결책·남은 가설을 정리했지만 진행하지 못하는 경우. 파일을 아직 읽지 않았거나 테스트를 아직 실행하지 않은 상태를 막힘으로 간주하지 않는다.
- **호출하지 않는 경우:** 일상적인 코드 작성, 단순 문서 편집, 알려진 오류 수정, 형식 정리, 기본 테스트 실행, 단순히 더 강한 모델을 선호하는 경우.

선택 기준은 운영상 제안이다. 범위가 분명한 코드·회귀 리뷰에는 Sol을, 원인이나 설계 경계가 복잡하고 근거가 충돌하는 문제에는 Astra를 우선 고려한다. 두 모델을 비교 호출해서 선택하지 않는다. 자문이 필요하지 않은 작업은 Luna Max만으로 완료한다.

### 하네스 실행 절차

1. **작업 정의:** 아래 M0~M5를 검토 가능한 작은 작업으로 나눈다. 작업마다 목표, 수정 파일 소유권, 의존성, 제외 범위, 관련 검사, 완료 조건을 기록한다.
2. **모델 설정 확인:** 주 실행자와 모든 Subagent의 실제 설정이 Luna Max인지 확인한다. 실행 환경이 확인 정보를 제공하지 않으면 확인 불가로 기록하고 설정이 검증됐다고 주장하지 않는다. 다른 모델로 조용히 대체하지 않는다.
3. **구현:** 주 실행자가 직접 처리하거나 Luna Max Subagent에 독립 범위를 배정한다. 같은 파일을 동시에 수정하지 않도록 소유권을 분리한다. 하위 작업에 필요한 근거만 전달한다.
4. **검증:** 변경에 맞는 테스트와 실제 산출물 검사를 수행한다. 테스트 실행 전후의 실패 원인을 구분하고 변경으로 생긴 실패를 해결한다.
5. **선택적 자문:** 위 조건을 충족한 경우에만 상위 모델 한 개에 한 작업을 의뢰한다. 리뷰할 커밋 또는 변경 스냅샷을 고정하고, 자문 중 그 범위는 수정하지 않는다.
6. **반영과 재검증:** Luna Max가 자문 의견을 근거와 대조해 채택·보류 사유를 기록하고 수정한다. 자문 답변 자체를 검증 통과로 간주하지 않는다.
7. **완료·다음 작업:** 산출물 존재, 완료 조건, 관련 검사, 남은 제한을 확인한 뒤 완료로 표시한다. 주 실행자가 Subagent 결과를 통합하고 다음 작업을 진행한다.

상위 모델이 제품 코드를 직접 수정하도록 권한을 주지 않는다. 읽기 전용 검토와 수정 방향·패치 제안은 가능하지만, 실제 파일 반영과 테스트는 Luna Max가 담당한다. 상위 모델 자문에서 추가 에이전트나 다른 모델을 호출하지 않는다.

### 상위 모델 의뢰 양식

다음 항목을 채운 뒤 의뢰한다. 결과는 결함·근거·최소 수정 방향·검증 제안 중심으로 받는다.

| 항목 | 기록 내용 |
| --- | --- |
| 의뢰 ID / 원 작업 ID | 예: REVIEW-IMP-03 / IMP-03 |
| 유형 / 선택 모델 | review 또는 blocker / Astra 또는 Sol 중 하나 |
| 단일 질문 | 이번 의뢰에서 답해야 할 한 가지 검토 목적 |
| 고정된 검토 대상 | 기준 커밋과 변경 diff 또는 보관한 스냅샷, 파일 목록 |
| 근거 | 재현 입력, 기대/실제 결과, 테스트 결과, 관련 로그 |
| Luna Max의 선행 작업 | 이미 확인한 내용, 시도한 해결책, 남은 가설 |
| 허용·금지 범위 | 읽기 전용 자문, 제품 코드 수정 금지, 다른 모델·에이전트 호출 금지 |
| 완료 조건 | 필요한 판단과 근거, 수정 방향, 추가 확인 항목 |

### 작업 기록과 종료 조건

구현을 시작하면 다음 열을 가진 작업 기록을 유지한다. 모델 이름은 의도한 설정과 실제 확인한 설정을 구분해 적는다.

| 작업 ID | 목표·파일 소유권 | 상태 | 실행자·실제 모델 설정 | 자문 ID·배정 모델 | 검증 증거 | 남은 제한 |
| --- | --- | --- | --- | --- | --- | --- |
| 미착수 | M0~M5 상세 작업은 구현 시작 시 분해 | 대기 | Luna Max 예정, 실행 설정 미확인 | 없음 | 이번 파서 조사 기록은 아래 참조 | 제품 구현 전 |

작업 상태는 대기 → 진행 → 검증 → 완료를 기본으로 하고, 필요할 때만 리뷰 대기 또는 막힘을 사용한다. Subagent가 종료됐다는 이유만으로 상위 작업을 완료하지 않는다. 재현되지 않은 개선 주장, 실행하지 않은 검사, 확인되지 않은 모델 설정은 완료 근거로 사용하지 않는다.

이 문서에 따른 구현 세션에서는 Luna Max Subagent 사용이 허용되지만 필수는 아니다. 모델 제약은 중첩 위임에도 전파한다. 사용자 지시 또는 현재 세션의 상위 규칙이 위임을 금지하면 그 규칙을 따르고 Luna Max 주 실행자가 직접 처리한다.
...
이번에 직접 실행한 검사는 **HTMLpoint의 실제 `parseReportHtml` 함수에 외부 HTML을 입력하는 호환성 검사**다. Vite SSR로 현재 소스를 로드하고 jsdom의 DOM API를 제공했다. 화면 렌더링·외부 스크립트 실행은 없는 파서 검사다. 최초 실행의 NodeFilter 전역 누락은 조사용 환경 문제였으며 전역을 추가한 뒤 완료했다. 결과는 [파서 검사 기록](docs/research/2026-09-18-reference-import-probe.json)에 남겼다.
...
근거: [제품 사양](docs/product-spec.md), [편집 명령](src/lib/editing.ts), [배치 선택](src/lib/objectLayout.ts), [파일 지속성](electron/filePersistence.ts), [검증 기록](QA_IMPROVEMENTS_2026-09-12.md). 위 기능을 새로 만드는 작업으로 다시 계획하지 않는다.
...
공수는 전담 개발자 1명 상당의 작업량, 기존 테스트 환경 활용, PowerPoint 검증 환경 확보를 가정한 초기 추정이다. Luna Max 및 선택적 Subagent 운영의 실제 경과 시간이나 토큰 사용량을 뜻하지 않으며, 상위 모델 자문에 의한 일정 단축을 전제하지 않는다. 1단계 후 재산정한다. 전체는 약 **35~54 작업일(7~11주)**이며 실제 현장 문서 대응 범위가 가장 큰 변수다.
...
2026-09-18 구현에서 다음 수직 슬라이스를 반영했다. 기존 원본 HTML을 `sourceHtml`과 section HTML로 유지하는 원칙은 계속 적용한다.

| 단계 | 이번 구현 결과 | 검증 상태 |
| --- | --- | --- |
| M0 | 기존 4개 bundled fixture의 parser/serializer 회귀를 유지하고 기본 release 검증 경로를 보존했다. | 자동 검증 완료. 실제 Windows 파일 창·IME·Office 시각 검증은 별도 환경에서 남음 |
| M1 | semantic/header·section, `.slide`·`.slide-portrait`·`data-slide`, `fig-canvas`, candidate adapter와 중첩 중복 제거, `sourceRef`·안정 object ID를 추가했다. | import adapter, 저장 후 재열기, candidate 선택 테스트 완료 |
| M2 | 문서 좌표 기반 render snapshot, text run·표·자산·도식 관계·revision guard와 editable/appearance PPTX PoC를 추가했다. | 실제 PPTX ZIP 생성 및 경고/실패 테스트 완료. PowerPoint에서의 수동 편집 확인은 남음 |
| M3 | PPTX Export UI, 진행 상태, 16:9·4:3·A4·source 비율, editable/fallback 보고서, atomic 저장을 추가했다. | renderer/unit/build 검증 완료. 취소 전용 UI와 12사례 Office 출력 회귀는 남음 |
| M4 | 도형·연결선, 관계 기반 connector, 복제·삭제·layer·group/ungroup·clipboard·Undo와 preview connector 재계산을 추가했다. | parser/editing/clipboard/serialization 테스트 완료 |
| M5 | 범위 텍스트 서식, 품질 패널, 배포 HTML, PDF printToPDF 경로를 추가했다. | serializer/file-service 테스트 완료. 실제 Windows IME/PDF/썸네일 시각 검증은 남음 |

검증 기준은 `npm test`, `npm run build`, `npm run build:electron`, `git diff --check`로 고정한다. 실제 Windows 파일 선택 창, Microsoft IME 조합, 설치본, PowerPoint의 재편집/저장 결과는 이 저장소의 jsdom/Vite 검사만으로 대체하지 않는다.
